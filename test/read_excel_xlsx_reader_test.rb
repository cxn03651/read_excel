...
class ReadExcelXLSXReaderTest < Minitest::Test
  def setup
    @filepath = File.join(File.dirname(File.expand_path(__FILE__)), 'testfile.xlsx')
    @workbook = nil
  end
  ...
  private
    ...
    def workbook
      @workbook ||= ReadExcel::XLSXReader.new(@filepath)
    end
end
